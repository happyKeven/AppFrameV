...
public class IDUtil {
    ...
    public static boolean idcardCheck(Context context, String idNum) {
        ...
        System.out.println("idNumLength ====== " + idNumLength);
        if (idNumLength != 15 && idNumLength != 18) {
            toastUtils.showToast("身份证号码应该为15位或18位");
            return false;
        }
        //对身份证的字符做判断
        if (!isAllNum(idNum)) {
            toastUtils.showToast(idNum.length() == 18 ? "18位号码除最后一位外,都应为数字(最后一位如果是字母,注意大小写)" : "15位号码都应为数字");
            return false;
        }

        if (idNum.contains("x")) {
            toastUtils.showToast("身份证x必须为大写");
            return false;
        }
        ...
        if (h.get(idNum.substring(0, 2)) == null) {
            toastUtils.showToast("身份证地区编码错误");
            return false;
        }
        ...
        try {
            if ((gc.get(Calendar.YEAR) - Integer.parseInt(strYear)) > 150 || (gc.getTime().getTime()
                ...
                toastUtils.showToast("身份证生日不在有效范围");
                return false;
            }
        ...
        if (idNum.length() == 18 && !isCorrectID(idNum)) {
            toastUtils.showToast("身份证无效,不是合法的身份证号码");
            return false;
        }
